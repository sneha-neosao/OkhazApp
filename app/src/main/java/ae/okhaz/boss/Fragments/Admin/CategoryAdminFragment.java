...
import ae.okhaz.boss.Adapter.Admin.AdminOrderAdapter1;
import ae.okhaz.boss.Fragments.BottomSheetFragment;
...
public class CategoryAdminFragment extends Fragment implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    AdminOrderAdapter1 orderAdapter;
    ...
    String condition ="",  OrderID ="",  startDate ="",   endDate ="",deliveryMan="",branchID="";
    ...
    String sort="",dateFrom="",dateTo="",startDateFilter="",endDateFilter="";

    private int mStartYear, mStartMonth, mStartDay, mHour, mMinute,mEndYear, mEndMonth, mEndDay;

    Dialog progressDialog;

    public static CategoryAdminFragment newInstance(String name, int position) {
        Bundle args = new Bundle();
        args.putString("code", name);
        args.putInt("position", position);
        CategoryAdminFragment f = new CategoryAdminFragment();
        f.setArguments(args);
        return f;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_category_admin, container, false);
        ...
        //SupplierActivity.toolbar.setTitle("Order List");
        /*SupplierActivity.txt_mainOrder_number.setVisibility(View.GONE);
        SupplierActivity.toolbar.setNavigationIcon(R.drawable.ic_menu);
        SupplierActivity.toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SupplierActivity.drawer.openDrawer(Gravity.LEFT);
               // ((SupplierActivity) getActivity()).loadFragment(new SupplierHomeFragment());
               // SupplierActivity.toolbar.setTitle("DashBoard");
            }
        });*/
...
//                getOrdersData(deliveryMan,branchID,nams,"0","","","","");
//
//            }
//        });
       ...
       /* SupplierActivity.toolbar.setNavigationOnClickListener(new View.OnClickListener() {
        ...
        if(user.get(SessionManagement.KEY_LOIGIN_STATUS).toString().equals("0"))
        {
            ...
            //Tools.loading(getActivity()).show();
            progressDialog.show();
            getOrdersData(deliveryMan,branchID,nams,"0","","",startDate,endDate);
        }
        else {
            offline_layout.setVisibility(View.VISIBLE);
            order_recycler.setVisibility(View.GONE);
                ...
                if (orderArrayList.size()>0)
                {
                   ...
                   // SupplierActivity activity = (SupplierActivity) view.getContext();
                    ...
                    activity.getSupportFragmentManager()
                            .beginTransaction().replace(R.id.nav_supplier_fragment, myFragment)
...
//        }
      ...
      //  shortRadioGroup = viewShortBottomSheet.findViewById(R.id.rgShort);
        dialogShortBottomSheet = new BottomSheetDialog(getActivity(),R.style.CustomBottomSheetStyle);
        ...
        dialogFilterBottomSheet = new BottomSheetDialog(getActivity(),R.style.CustomBottomSheetStyle);
        dialogFilterBottomSheet.getWindow().setBackgroundDrawable(new ColorDrawable(getResources().getColor(R.color.transparent_color)));
        ...
        filter_tv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
              ...
              /*  BottomSheetFragment bottomSheetFragment = new BottomSheetFragment();
                ...
                bundle.putString("name", nams);
                bottomSheetFragment.setArguments(bundle);
                BottomSheetDialogFragment bottomSheetDialogFragment = bottomSheetFragment;
                bottomSheetDialogFragment.show(getChildFragmentManager(), String.valueOf(pos));
               */
                bottomSheetDialog=new Dialog(getContext(),R.style.CustomDialog);
                ...
                /*Set default DateFrom to one week before date*/
                    ...
                    /*String dateOneWeekBefore = getCalculatedDate("dd/MM/yyyy",-7);
                    dateFrom = getCalculatedDate("yyyy/MM/dd",-7);*/
              ...
              /*  dialogFilterBottomSheet.setContentView(viewFilterBottomSheet);
                dialogFilterBottomSheet.getWindow().setBackgroundDrawableResource(R.color.white_trans);
                dialogFilterBottomSheet.show();*/
            }
        });
    ...
    private void getOrdersData(String deliveryman, String BranchCode, String OrderStatus, String offset, String condition, String OrderID, String startDate,  String endDate) {
        ...
        ServiceGenerator.getDelivery().getOwnerOrders(ownerID,deliveryMan,
                BranchCode,role,OrderStatus, offset, condition, OrderID, startDate,
                endDate,date_range).enqueue(new Callback<ResponseOrderAdminList>() {
            @Override
...
//                        }
                        ...
                        {
                            Log.e("CateAdminFrag brodcast",e.getMessage());
                        }
                        if (orderArrayList.size()>0)
                        {
    ...
    private void setRecycler(ArrayList<Order> orderArrayList) {
        ...
        orderAdapter = new AdminOrderAdapter1(orderArrayList, getContext(),pos,0);
...
//                    ((MainActivity)getActivity()).toolbar.setVisibility(View.VISIBLE);
    ...
    private void loadMore(final int scrollPosition, String deliveryman, String branchCode, String orderStatus, String condition, String orderID, String startDate, String endDate) {
        ...
        ServiceGenerator.getDelivery().getOwnerOrders(ownerID,deliveryman, branchCode,role,orderStatus,""+ scrollPosition, condition, OrderID, startDate, endDate,date_range).enqueue(new Callback<ResponseOrderAdminList>() {
            @Override
            public void onResponse(Call<ResponseOrderAdminList> call, Response<ResponseOrderAdminList> response) {
                if (response.isSuccessful())
                {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if(wall.equals("0"))
        {
            ...
            offline_layout.setVisibility(View.GONE);
            order_recycler.setVisibility(View.VISIBLE);
            getOrdersData(user.get(SessionManagement.KEY_DELIVERY_MAN).toString(),
                    user.get(SessionManagement.KEY_BRANCH_ID).toString(),nams,"0",condition,OrderID,startDate, endDate);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (bundle!= null)
            {
                ...
                if (type.equals("filter"))
                {
                    ...
                    else  if (status.equals("Ready For PickUp"))
                    {
                        nams="4";
                    }
                    else  if (status.equals("Oder In Progress"))
                    {
                        nams="5";
                    }
                    else  if (status.equals("Order on the way"))
                    {
                        nams="6";
                    }
                    ...
                    else  if (status.equals("Order Rejected"))
                    {
                        nams="10";
                    }
                    ...
                    else  if (status.equals("Awaiting Payment"))
                    {
                        nams="12";
                    }
                    else  if (status.equals("Awaiting Fulfillment"))
                    {
                        nams="13";
                    }
                    else  if (status.equals("Awaiting Shipment"))
                    {
                        nams="14";
                    }
                    else  if (status.equals("High Risk"))
                    {
                        nams="15";
                    }
                    else  if (status.equals("Pre-Orders"))
                    {
                        nams="16";
                    }
                    ...
                    else  if (status.equals("Partial Confirmed"))
                    {
                        nams="21";
                    }


                    getOrdersData( deliveryMan,  branchID, nams, "0",  condition,  OrderID, startDate,  endDate);
                }
